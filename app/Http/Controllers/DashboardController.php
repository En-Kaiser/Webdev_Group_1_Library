...
class DashboardController extends Controller
{
    ...
    public function studentViewAll(Request $request)
    {
        ...
        $books = $query->select('books.*', 'a.name as author', 'g.genre_id', 'g.name as genre')->get();
    ...
    {
        $query = book::query()
            ->join('bookmarks as bm', 'bm.book_id', '=', 'books.book_id')
            ->leftJoin('books_joint_authors as bja', 'bja.book_id', '=', 'books.book_id')
            ->leftJoin('authors as a', 'a.author_id', '=', 'bja.author_id')
            ->leftJoin('books_joint_genres as bjg', 'bjg.book_id', '=', 'books.book_id')
    ...
    {
        $query = history::query()
            ->join('books as b', 'b.book_id', '=', 'history.book_id')
            ->where('history.user_id', Auth::id());

        if ($request->has('status') && $request->status !== 'all') {
            $query->where('history.status', $request->status);
        }
